...
def GET_SCALE_FACTOR(j, c, waveL_range, stellar_spectra, u_mag = False):
    ...
    if V_jmag == 0:
        ...
        cross_sec_interp = interp1d(cross_sec['wavelength'], cross_sec['c_section'], bounds_error=False, fill_value="extrapolate")

        scale = 3.64e-9 * pow(10, -0.4 * (V_jmag - 3.1 * ebv))  / vflux #* 4 * math.pi
        tau = cross_sec_interp(waveL_range) * ebv * gas_to_dust
...
for j in range (5): 
    ...
    if names[target_hips.index(df['hip'][j])] == 'Sirius':
        ...
        df2 = pd.read_csv(file_path, delimiter=r',', header=None, names=['wavelength', 'flux'])
        # print(df2.head())
        ...
        # plt.savefig(fr'Stellar_Spectra\Test_Stars\{names[j]}_spectrum.png')
        # Show the plot
        # plt.show()


#     # --- Load Johnson U filter ---
    U_filter = np.loadtxt("Stellar_Spectra/Johnson_U.dat")   # wavelength [Å], transmission
...
#     u_mag2 = "--x" #-2.5 * np.log10( tot_flux[uindex2] / 3.006893995802142e-09)
    ...
    print(f"{names[j]} (HIP-{df['hip'][j]}) \n U band magnitude: Integrated: {u_mag}")#,   observed: {u_mag1},   Simulated: {u_mag2}")
